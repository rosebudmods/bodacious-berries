...
public class JuicerScreen extends HandledScreen<JuicerScreenHandler> {
    ...
    private void drawProgress(GuiGraphics graphics) {
        ...
        if (brewTime > 0) {
            ...
            if (progress > 0) {
                graphics.method_52708(RenderLayer::getGuiTextured, TEXTURE,
                        x + 73, y + 35, 0, 0,
                        dubious ? DUBIOUS_PROGRESS_BAR_UV_X : PROGRESS_BAR_UV_X,
                        blend ? BLEND_PROGRESS_BAR_UV_Y : PROGRESS_BAR_UV_Y,
                        28, progress);
            }
            ...
            if (progress > 0) {
                for (int i = 0; i < 2; i++) {
                    graphics.method_52708(RenderLayer::getGuiTextured, TEXTURE,
                            x + (i == 0 ? LEFT_BUBBLE_X : RIGHT_BUBBLE_X),
                            y + 63 - progress,
                            0, 0,
                            dubious ? DUBIOUS_BUBBLE_UV_X : BUBBLE_UV_X,
                            (blend ? BLEND_BUBBLE_UV_Y : BUBBLE_UV_Y) - progress,
                            12, progress);
                }
            }
        }
